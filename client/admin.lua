...
RegisterNUICallback('craftadminTaxonomyAudit', function(_, cb)
    cb(lib.callback.await('sanctuary_crafting:craftadminTaxonomyAudit', false) or { ok = false })
end)
RegisterNUICallback('craftadminSuggestClass', function(data, cb)
    cb(lib.callback.await('sanctuary_crafting:craftadminSuggestClass', false, data and data.recipeId) or { ok = false })
end)
RegisterNUICallback('craftadminBulkMove', function(data, cb)
    cb(lib.callback.await('sanctuary_crafting:craftadminBulkMove', false, data) or { ok = false })
end)
RegisterNUICallback('craftadminApplySuggestions', function(data, cb)
    cb(lib.callback.await('sanctuary_crafting:craftadminApplySuggestions', false, data) or { ok = false })
end)
RegisterNUICallback('craftadminUpsertCategory', function(data, cb)
    cb(lib.callback.await('sanctuary_crafting:craftadminUpsertCategory', false, data) or { ok = false })
end)
RegisterNUICallback('craftadminUpsertSubcategory', function(data, cb)
    cb(lib.callback.await('sanctuary_crafting:craftadminUpsertSubcategory', false, data and data.catUid, data and data.def) or { ok = false })
end)
